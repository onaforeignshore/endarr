"""Webhook handler for *Arr events (Grab, Download, Test)."""
...
def format_kv(key: str, value: Any) -> str:
    """Format a key=value pair with cyan colouring.
    ...
    Returns:
        Coloured string like "{cyan}key=value{reset}".
    """
    return f"{{cyan}}{key}{{reset}}={value}"


def handle_arr_webhook():
    """Process incoming webhook from *Arr (Radarr, Sonarr, Lidarr).
    ...
    if "eventType" in data:
        ...
        if event_type == "Test":
            # Identify which *Arr sent the test
            if "movie" in data:
                arr_name = "Radarr"
            elif "series" in data:
                arr_name = "Sonarr"
...
def handle_grab(data: Dict[str, Any]):
    """Process a Grab event.
...
def handle_download(data: Dict[str, Any]):
        ...
        if is_upgrade and media_id:
            ...
            if old_download:
                ...
                for name, instance in client_instances_by_name.items():
                    if name == old_download.client_id:
